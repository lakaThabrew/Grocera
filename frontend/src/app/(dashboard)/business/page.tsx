      ...
      <PageWrapper>
        <div className="text-center p-16 glass-card rounded-xl border border-dashed border-destructive/30 max-w-2xl mx-auto">
          ...
          <p className="text-muted-foreground mt-2">
            The Business Dashboard is exclusively available to registered
  ...
  return (
    <PageWrapper className="max-w-7xl mx-auto w-full">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-8">
        ...
        <button
          onClick={() =>
            heatmap && exportToCSV(heatmap.data, "Competitor_Heatmap")
          }
        ...
        >
          ...
          <div className="flex flex-wrap gap-4">
            {marketShare?.competitorShare.map((comp) => (
              <div
                key={comp.name}
                className="flex-1 min-w-[120px] bg-muted/30 p-4 rounded-lg border border-border"
              >
        ...
        >
          ...
          {heatmap ? (
            <div className="min-w-[600px]">
                ...
                <thead className="text-xs uppercase bg-muted/50 text-muted-foreground">
                  <tr>
                    ...
                    {heatmap.competitors.map((comp) => (
                      <th key={comp} className="px-4 py-3 text-center">
                        {comp}
                      </th>
                    ))}
                  </tr>
                      ...
                      {heatmap.competitors.map((comp) => {
                        ...
                        return (
                          <td key={comp} className="px-2 py-2">
                            <div
                              className={`w-full py-2 px-1 text-center rounded font-semibold ${getHeatmapColor(numericVal)}`}
                            >
                              {numericVal > 0 ? "+" : ""}
                              {numericVal}%
                            </div>
              ...
              <div className="flex items-center gap-6 mt-6 text-xs text-muted-foreground">
                <div className="flex items-center gap-2">
                  <div className="w-3 h-3 rounded bg-emerald-500/50"></div> We
                  are cheaper
                </div>
                <div className="flex items-center gap-2">
                  <div className="w-3 h-3 rounded bg-destructive/50"></div> We
                  are more expensive
                </div>
              </div>
            </div>
            ...
            <div className="h-[300px] flex items-center justify-center text-muted-foreground">
              <SearchX className="h-8 w-8 mr-2 opacity-50" /> No competitors
              found
            </div>
          )}
        </motion.div>
