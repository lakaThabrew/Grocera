...
export default function ScraperControlPage() {
  ...
  const [categoryUrl, setCategoryUrl] = useState(
    "https://www.keellssuper.com/product/category/all",
  );
      ...
      <PageWrapper>
        <div className="text-center p-16 glass-card rounded-xl border border-dashed border-destructive/30 max-w-2xl mx-auto">
          ...
          <p className="text-muted-foreground mt-2">
            The Scraper Engine control panel is restricted to system
            Administrators.
          </p>
        </div>
      </PageWrapper>
  ...
  const triggerScrape = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setMessage(
        res.data.message ||
          "Scraping job queued successfully! Products will appear soon.",
      );
    } catch (err: unknown) {
        ...
        <div className="p-6 border-b border-border bg-muted/30">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-sm text-muted-foreground">
                Push a new job into the BullMQ queue
              </p>
            </div>
          </div>
        </div>
