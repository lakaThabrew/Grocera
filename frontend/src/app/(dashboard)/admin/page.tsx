...
export default function AdminDashboard() {
  ...
  useEffect(() => {
    ...
    void Promise.all([
      ...
      api.get<{ data: UserRecord[] }>("/admin/users"),
      api.get<SystemHealth>("/admin/health"),
    ])
      .then(([statsResponse, usersResponse, healthResponse]) => {
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div className="bg-card p-6 rounded-xl border border-border">
              <h3 className="font-semibold mb-4 flex items-center gap-2">
                <Activity className="w-5 h-5 text-green-500" /> Process Memory
                (RSS)
              </h3>
              <div className="w-full bg-muted rounded-full h-4 mb-2 overflow-hidden">
                <div
                  ...
                  style={{
                    width: `${Math.min((health.memory.rss / health.os.totalmem) * 100 * 10, 100)}%`,
                  }}
                ></div>
              </div>
              <p className="text-sm text-muted-foreground">
                {(health.memory.rss / 1024 / 1024).toFixed(2)} MB Used
              </p>
            </div>

            <div className="bg-card p-6 rounded-xl border border-border">
              <h3 className="font-semibold mb-4 flex items-center gap-2">
                <Server className="w-5 h-5 text-blue-500" /> System Uptime
              </h3>
      ...
      {activeTab === "QUEUES" && (
        <div className="bg-card rounded-xl border border-border overflow-hidden p-8 text-center flex flex-col items-center justify-center min-h-[300px]">
          ...
          <p className="text-muted-foreground max-w-md mx-auto mb-6">
            Monitor and manage background scraping jobs. The queue dashboard is
            protected by Basic Authentication. Use your admin credentials to log
            in.
          </p>
          <a
