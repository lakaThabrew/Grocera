...
export default function AlertsPage() {
  ...
  const toggleAlert = async (id: string, currentStatus: boolean) => {
    try {
      await api.patch(`/consumers/alerts/${id}`, { isActive: !currentStatus });
      setAlerts(
        alerts.map((a) =>
      ...
      {isLoading ? (
        ...
        <div className="text-center p-12 glass-card rounded-xl border border-dashed border-white/10">
          ...
          <p className="text-muted-foreground mt-1 max-w-sm mx-auto">
            Find products you love and click the bell icon to get notified when
            the price drops below your target!
          </p>
          <Link
            href="/products"
            className="inline-block mt-6 px-6 py-2 bg-primary text-primary-foreground rounded-md font-medium hover:bg-primary/90 transition-colors"
          >
            Browse Products
          </Link>
        </div>
                ...
                >
                  ...
                  <div className="grid grid-cols-2 gap-4 mb-4">
                    ...
                    <div className="bg-primary/5 p-3 rounded-lg border border-primary/10">
                      <p className="text-[10px] uppercase text-primary/80 font-medium mb-1">
                        Target
                      </p>
                      <p className="font-bold text-lg text-primary">
                        Rs. {alert.targetPrice}
                      </p>
                    </div>
                  </div>
                  ...
                  <div className="flex items-center gap-3 mb-5 text-muted-foreground">
                    <div className="flex gap-2">
                      {alert.emailAlert && (
                        <span title="Email Alerts On">
                          <Mail className="h-4 w-4" />
                        </span>
                      )}
                      {alert.pushAlert && (
                        <span title="Push Alerts On">
                          <Smartphone className="h-4 w-4" />
                        </span>
                      )}
                      {alert.smsAlert && (
                        <span title="SMS Alerts On">
